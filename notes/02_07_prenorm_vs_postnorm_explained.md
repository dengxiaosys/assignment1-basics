...
本讲义与 [bpe/01_02_byte_level_bpe_worked_example.md](./bpe/01_02_byte_level_bpe_worked_example.md) 属于同一套 CS336 学习笔记，符号风格保持一致。它讲的是"数据如何进入模型"，本文讲的是"数据进入模型后，每一层如何保持数值与梯度的稳定"。
...
**一个有用的对照**：$g$ 是**每个 norm 块独立**的可学习参数；而 RoPE 的旋转频率 $\theta_i$（见配套 [02_08_rope_explained.md](./02_08_rope_explained.md)）是**全模型共享且不可学习**的固定值。两者都作用在"逐通道/逐维度"层面，但可学习性与共享范围恰好相反，可对照记忆。
